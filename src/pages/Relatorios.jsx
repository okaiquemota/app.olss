...
import { 
  ...
  Activity, ArrowLeft, Search, Plus, Loader2, Calendar 
} from 'lucide-react';
...
export function Relatorios() {
  ...
  const handleGerarRelatorio = async () => {
    ...
    try {
      ...
      doc.setDrawColor(200, 200, 200); // Corrigido de setDrawingColor para setDrawColor
      ...
      doc.text(`${(clienteSelecionado.nome_razao_social || '').toUpperCase()}`, 38, currentY);

      currentY += 6;
      doc.setFont("helvetica", "bold");
      doc.text(`ENDEREÇO:`, 14, currentY);
      doc.setFont("helvetica", "normal");
      doc.text(`${(clienteSelecionado.endereco || 'Endereço não informado').toUpperCase()}`, 38, currentY);

      currentY += 6;
      doc.setFont("helvetica", "bold");
      doc.text(`REFERÊNCIA:`, 14, currentY);
      doc.setFont("helvetica", "normal");
      doc.text(`${mesReferencia.toUpperCase()}`, 38, currentY);

      // Seção 1: Descrição de Resultado
      currentY += 10;
      doc.setFont("helvetica", "bold");
      doc.setTextColor(21, 128, 61);
      ...
      const gAtual = parseNum(geracaoAtual);
      const gAnt = parseNum(geracaoAnterior);
      ...
      // Seção 2: Geração e Comparações
      doc.setFont("helvetica", "bold");
      doc.setTextColor(21, 128, 61);
      doc.text(`1. GERAÇÃO - ${mesReferencia.toUpperCase()} - ${gAtual} KWH/MÊS`, 14, currentY);
      ...
      // Seção 3: Relação da Conta Geradora
      ...
      if (geradoraObj) {
        ...
        currentY += 5;
        const calcGeradora = calcularLinha(geradoraObj);
        doc.text(`• Valor auto consumo: ${calcGeradora.autoConsumo.toFixed(0)} kWh/mês`, 18, currentY);
        currentY += 5;
        doc.text(`• Consumo da conta: ${parseNum(geradoraObj.consumoConta)} kWh/mês`, 18, currentY);
      }

      // Seção 4: Valores das Contas (Tabela Completa)
      currentY += 10;
      doc.setFont("helvetica", "bold");
      doc.setTextColor(21, 128, 61);
      ...
      const tableData = unidades.map(u => {
        const calc = calcularLinha(u);
        return [
          `${u.tipo}\nUC: ${u.numero_uc}`,
          `${calc.consumoTotal.toFixed(0)} kWh`,
          `R$ ${parseNum(u.precoKwh).toFixed(2).replace('.', ',')}`,
          `R$ ${parseNum(u.tarifaConta).toFixed(2).replace('.', ',')}`,
          `R$ ${calc.valorSemSistema.toFixed(2).replace('.', ',')}`,
          `R$ ${parseNum(u.valorConta).toFixed(2).replace('.', ',')}`,
          `R$ ${calc.economia.toFixed(2).replace('.', ',')}`
        ];
      });
      ...
      // Seção 5: Resolução Final / Parecer Técnico
      currentY += 10;
      doc.setFont("helvetica", "bold");
      doc.setFontSize(10);
      doc.setTextColor(21, 128, 61);
      ...
      doc.setFontSize(9);
      doc.setTextColor(60, 60, 60);
      const textoParecer = parecerTecnico || "Resultado dentro do esperado para o período, com contas de energia atenuadas ao limite tarifário mínimo.";
      ...
      doc.save(`Relatorio_${clienteSelecionado.nome_razao_social.substring(0, 15)}_${mesReferencia.replace('/', '-')}.pdf`);
