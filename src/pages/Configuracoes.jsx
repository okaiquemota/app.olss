...
import { Plus, Trash2, ExternalLink, Loader2, LayoutGrid, AlertCircle, X, AlertTriangle } from 'lucide-react';
...
export function Configuracoes() {
  ...
  // Esc fecha a confirmação de exclusão
  useEffect(() => {
    if (!plataformaParaExcluir) return;
    const aoTeclar = (e) => { if (e.key === 'Escape') setPlataformaParaExcluir(null); };
  ...
  const handleAdd = async (e) => {
    ...
    // deixaria o link ambíguo
    const jaExiste = plataformas.some(p => p.nome?.trim().toLowerCase() === nomeLimpo.toLowerCase());
    if (jaExiste) {
    ...
    const { error } = await supabase
      .from('plataformas')
      .insert([{ nome: nomeLimpo, link: linkLimpo ? linkSeguro(linkLimpo) : '' }]);
    setSalvando(false);

    if (error) {
      setErro('Não foi possível cadastrar a plataforma. Tente novamente.');
      return;
    }
          ...
          <div>
            <form onSubmit={handleAdd} className="flex flex-col sm:flex-row gap-2">
              ...
              <button
                ...
                className="flex items-center justify-center gap-1.5 bg-green-700 text-white px-4 py-2 rounded-none text-[12px] font-bold hover:bg-green-800 transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed min-w-[100px]"
              >
                ...
                return (
                  <div key={plat.id} className="flex justify-between items-center gap-3 px-3 py-2.5 hover:bg-gray-100 transition-colors">
                    ...
                    <button
                      ...
                      className="text-gray-500 hover:text-red-600 transition-colors p-1.5 hover:bg-red-50 border border-transparent hover:border-red-200 rounded-none cursor-pointer shrink-0"
                      title="Remover"
