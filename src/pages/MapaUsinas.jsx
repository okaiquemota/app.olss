...
export function MapaUsinas() {
  ...
  if (loading) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center bg-gray-50 border border-gray-300 rounded-none text-gray-500">
  ...
  return (
    <div className="-m-3 md:-m-4 h-[calc(100vh-48px)] relative z-0 overflow-hidden animate-in fade-in duration-300 bg-gray-100 border border-gray-300">
      ...
      <div className="absolute top-3 left-3 md:top-4 md:left-4 z-[1000] w-[calc(100vw-1.5rem)] sm:w-80 md:w-96 pr-3 sm:pr-0">
        <div className="relative">
          ...
          <input 
            ...
            className="w-full pl-9 pr-4 py-2 bg-white border border-gray-300 rounded-none text-[13px] font-medium text-gray-800 focus:outline-none focus:ring-1 focus:ring-green-500 focus:border-green-500 shadow-sm placeholder:text-gray-400 transition-all"
          />
        </div>
      </div>

      {/* LEGENDA DUPLA (Preenchimento vs Contorno) */}
      <div className="hidden sm:flex flex-col absolute top-3 right-3 md:top-4 md:right-4 z-[1000] bg-white border border-gray-300 p-3 rounded-none shadow-sm gap-2">
        <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1 flex items-center gap-1"><DollarSign size={12}/> Preenchimento (Comercial)</div>
        <div className="grid grid-cols-2 gap-y-2 gap-x-4">
          <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-green-500"></div><span className="text-[10px] font-bold text-gray-600 uppercase tracking-wider">Contrato</span></div>
          <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-red-500"></div><span className="text-[10px] font-bold text-gray-600 uppercase tracking-wider">S/ Contrato</span></div>
          <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-yellow-500"></div><span className="text-[10px] font-bold text-gray-600 uppercase tracking-wider">Prospecção</span></div>
        </div>
        
        <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-1 mb-1 border-t border-gray-100 pt-2 flex items-center gap-1">
          <Activity size={12}/> Contorno (Apenas c/ contrato)
        </div>
        <div className="grid grid-cols-2 gap-y-2 gap-x-4">
          <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full border-[2px] border-green-600 bg-transparent"></div><span className="text-[10px] font-bold text-gray-600 uppercase tracking-wider">Normal</span></div>
          <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full border-[2px] border-[#FFFF00] bg-transparent"></div><span className="text-[10px] font-bold text-gray-600 uppercase tracking-wider">Offline</span></div>
          <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full border-[2px] border-[#FFC000] bg-transparent"></div><span className="text-[10px] font-bold text-gray-600 uppercase tracking-wider">Problema</span></div>
          <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full border-[2px] border-[#FF0000] bg-transparent"></div><span className="text-[10px] font-bold text-gray-600 uppercase tracking-wider">Vencido</span></div>
        </div>
      </div>

      {/* CARD DE DETALHES FLAT */}
      {usinaSelecionada && (
        <div className="absolute bottom-3 left-3 md:bottom-4 md:left-4 z-[1000] w-[calc(100vw-1.5rem)] sm:w-80 md:w-96 bg-white border border-gray-300 p-4 rounded-none shadow-sm animate-in slide-in-from-bottom-4 duration-200">
          <button 
            onClick={() => setUsinaSelecionada(null)} 
            className="absolute top-3 right-3 text-gray-400 hover:text-red-600 hover:bg-red-50 p-1.5 transition-colors cursor-pointer rounded-none border border-transparent hover:border-red-200"
          >
          ...
          <div className="flex items-start gap-3 mb-4 pr-6">
            ...
            <div>
              ...
              {usinaSelecionada.endereco && (
                <span className="text-[11px] font-medium text-gray-500 line-clamp-2 leading-tight">
                  {usinaSelecionada.endereco}
                </span>
              )}
            </div>
          ...
          {usinaSelecionada.obs && (
            <div className="mb-4 bg-gray-50 p-3 rounded-none border border-gray-200">
              <span className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Anotações</span>
          ...
          <div className="pt-3 border-t border-gray-200">
            <a 
              ...
              target="_blank"
              rel="noreferrer"
              className="w-full flex items-center justify-center gap-2 bg-white border border-gray-300 text-gray-700 py-2 rounded-none text-[12px] font-bold hover:bg-gray-50 transition-all cursor-pointer"
            >
